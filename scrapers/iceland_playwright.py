"""
Iceland scraper using Playwright with API response interception.
Captures JSON responses from Iceland's e-commerce platform.
Iceland specialises in frozen food and home delivery.
"""
...
class IcelandPlaywrightScraper:
    """Scraper for Iceland using Playwright + API interception."""
    ...
    SKIP_URL_FRAGMENTS = (
        ...
        "basket", "trolley", "delivery", "checkout",
    ...
    def _on_response(self, response: PWResponse) -> None:
        ...
        try:
            ...
            url_match = any(p.lower() in url.lower() for p in self.PRODUCT_API_PATTERNS)
        ...
        def _scan(obj, depth=0):
            ...
            if isinstance(obj, list) and len(obj) >= 2:
                ...
                if isinstance(first, dict):
                    keys = set(first.keys())
                    product_keys = {
                        "name", "title", "displayName", "productName",
                        "price", "salePrice", "listPrice", "retail_price",
                        "sku", "productId", "gtin", "ean", "barcode",
    ...
    def _looks_like_product(item: dict) -> bool:
        keys = set(item.keys())
        indicators = {
            "name", "title", "displayName", "productName", "sku",
            "price", "salePrice", "listPrice",
            "gtin", "ean", "barcode", "image", "imageUrl",
        }
    ...
    def _api_item_to_product(self, item: dict) -> Optional[Dict[str, Any]]:
        ...
        gtin = item.get("gtin") or item.get("ean") or item.get("barcode") or None
        if gtin:
            gtin = str(gtin).strip()
        ...
        # Was price
        was_price = None
        was_val = item.get("was") or item.get("wasPrice") or item.get("listPrice") or item.get("previousPrice")
        if isinstance(was_val, (int, float)) and was_val > price:
    ...
    def _extract_price(item: dict) -> Optional[float]:
        for key in ("price", "salePrice", "retail_price", "displayPrice", "currentPrice"):
            val = item.get(key)
        ...
        if isinstance(price_info, dict):
            for key in ("price", "now", "selling_price", "sale"):
    ...
    def _parse_html_fallback(self, page: Page, max_items: int) -> List[Dict[str, Any]]:
        ...
        # Try __NEXT_DATA__ (Iceland may use Next.js)
        try:
        ...
        with sync_playwright() as playwright:
            ...
            try:
                ...
                if not products and self.debug:
                    ...
                    try:
                        page.screenshot(path="iceland_debug_screenshot.png")
                        print("  📸 Debug screenshot saved")
                    except Exception:
                        pass

                print(f"✓ Scraped {len(products)} products from Iceland")

            except Exception as e:
                print(f"✗ Error scraping Iceland: {e}")
...
def main():
    scraper = IcelandPlaywrightScraper(headless=False)
