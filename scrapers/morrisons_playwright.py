"""
Morrisons scraper using Playwright with API response interception.
Captures JSON responses from Morrisons' internal REST APIs during
page navigation, same pattern as the Asda scraper.
"""
...
class MorrisonsPlaywrightScraper:
    """Scraper for Morrisons using Playwright + API interception."""
    ...
    SKIP_URL_FRAGMENTS = (
        ...
        "trolley", "basket", "timeslots", "delivery", "store-finder",
            ...
            if isinstance(obj, list) and len(obj) >= 2:
                ...
                if isinstance(first, dict):
                    ...
                    product_keys = {
                        ...
                        "gtin", "ean", "sku", "skuId", "productId",
                        "catchWeight", "image", "brandName",
                    }
    ...
    def _find_product_arrays(self, obj, depth: int = 0) -> List[List]:
        ...
        if isinstance(obj, dict):
            for key in ("products", "items", "results", "searchResults",
                        "hits", "data", "fops", "productItems", "entries"):
    ...
    def _looks_like_product(item: dict) -> bool:
        keys = set(item.keys())
        indicators = {
            "name", "title", "displayName", "sku",
            "price", "priceInfo", "retail_price",
            "gtin", "ean", "image", "brandName",
        }
    ...
    def _api_item_to_product(self, item: dict) -> Optional[Dict[str, Any]]:
        ...
        name = (
            ...
            item.get("displayName") or item.get("product_name") or ""
        ).strip()
        if not name:
            return None
        ...
        unit_price = (
            item.get("unit_price") or item.get("unitPrice") or
        ...
        gtin = item.get("gtin") or item.get("ean") or item.get("sku") or None
        if gtin:
            gtin = str(gtin).strip()
    ...
    def _extract_price(item: dict) -> Optional[float]:
        for key in ("price", "retail_price", "listPrice", "basePrice"):
            val = item.get(key)
        ...
        if isinstance(price_info, dict):
            for key in ("price", "now", "retail", "selling_price"):
    ...
    def _extract_member_price(item: dict) -> Optional[str]:
        for key in ("moreCardPrice", "memberPrice", "member_price", "loyaltyPrice"):
            val = item.get(key)
            if isinstance(val, (int, float)) and val > 0:
                return str(val)
            if isinstance(val, str):
        ...
        loyalty = item.get("loyalty") or item.get("moreCard") or {}
        if isinstance(loyalty, dict):
            val = loyalty.get("price") or loyalty.get("value")
            if isinstance(val, (int, float)) and val > 0:
                return str(val)
        ...
        if isinstance(promos, list):
            for p in promos:
                if isinstance(p, dict):
                    ...
                    if "more card" in desc or "more points" in desc:
        ...
        with sync_playwright() as playwright:
            ...
            try:
                ...
                if not products and self.debug:
                    ...
                    try:
                        page.screenshot(path="morrisons_debug_screenshot.png")
                        print("  📸 Debug screenshot saved")
                    except Exception:
                        pass

                print(f"✓ Scraped {len(products)} products from Morrisons")

            except Exception as e:
                print(f"✗ Error scraping Morrisons: {e}")
...
def main():
    scraper = MorrisonsPlaywrightScraper(headless=False)
